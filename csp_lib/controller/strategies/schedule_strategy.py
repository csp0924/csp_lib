...
from csp_lib.controller.core import Strategy, StrategyContext, Command, ExecutionConfig, ExecutionMode
...
class ScheduleStrategy(Strategy):
    ...
    def on_activate(self) -> None:
        """啟用排程策略"""
        logger.info(f"ScheduleStrategy: 排程策略啟用")
        self.current_strategy.on_activate()
    
    def on_deactivate(self) -> None:
        """停用排程策略"""
        logger.info(f"ScheduleStrategy: 排程策略停用")
