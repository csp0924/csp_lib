...
Equipment 模組是 csp_lib 的核心抽象層，建構於 [[_MOC Modbus]] 與 [[_MOC CAN]] 之上，提供完整的非同步設備管理能力。本層將底層 I/O（Modbus 暫存器 / CAN 訊框）封裝為高階設備模型，包含點位定義、資料轉換管線、告警系統、傳輸優化、以及事件驅動架構。兩種協定透過 [[DeviceProtocol]] 共用上層介面。
...
| 頁面 | 說明 |
|------|------|
| [[AsyncModbusDevice]] | Modbus 設備類別，整合讀寫、告警、事件、動態重配置 |
| [[AsyncCANDevice]] | CAN Bus 設備類別，整合 TX/RX、Snapshot Loop、RX Timeout |
...
| 頁面 | 說明 |
|------|------|
| [[CANEncoder]] | CAN 信號編碼與 Frame Buffer |
| [[PeriodicSendScheduler]] | CAN 訊框定期發送排程器 |
