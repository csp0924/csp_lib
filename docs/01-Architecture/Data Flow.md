...
設備週期性讀取的完整資料流，從排程到事件發射。v0.4.0 起新增 CAN 設備讀取路徑，與 Modbus 路徑並列運行。
...
flowchart TD
    ...
    subgraph CAN路徑["CAN 讀取路徑（v0.4.0）"]
...
從設備數據聚合到功率命令寫入的完整流程。v0.4.0 起加入 [[PowerDistributor]] 分支，支援 per-device 不同命令分配。
...
[[ModeManager]] 管理多模式優先級切換，支援基礎模式與覆蓋模式。v0.4.0 起，[[EventDrivenOverride]] 統一負責條件式 push/pop，取代原本硬編碼的 `_handle_auto_stop`。
...
5. **事件驅動** — v0.4.0 的 [[EventDrivenOverride]] 可自動依 context 條件 push/pop，含冷卻防抖
